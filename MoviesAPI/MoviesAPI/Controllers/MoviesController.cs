...
namespace MoviesAPI.Controllers
{
    ...
    public class MoviesController : ControllerBase
    {
        ...
        public async Task<ActionResult<HomeDTO>> Get()
        {
            ...
            var homeDTO =
                new HomeDTO {
                    ...
                    UpcomingReleases =
                        mapper.Map<List<MovieDTO>>(upcomingReleases)
                };
        ...
        public async Task<ActionResult<MovieDTO>> Get(int id)
        {
            var movie =
                await context
                    ...
                    .ThenInclude(Mvt => Mvt.MovieTheater)
        ...
        Filter([FromQuery] FilterMoviesDTO filterMoviesDTO)
        {
            ...
            if (!string.IsNullOrEmpty(filterMoviesDTO.Title))
            {
                moviesQueryable =
                    moviesQueryable
                        .Where(x => x.Title == filterMoviesDTO.Title);
            }
            ...
            if (filterMoviesDTO.UpcomingReleases)
            {
                var today = DateTime.Today;
                moviesQueryable =
                    moviesQueryable.Where(x => x.ReleaseDate > today);
            }
            ...
            await HttpContext
                .InsertParametersPaginationInHeader(moviesQueryable);

            var movies =
                await moviesQueryable
                    .OrderBy(m => m.Title)
        ...
        {
            var movieTheaters =
                await context.MovieTheaters.OrderBy(x => x.Name).ToListAsync();
            var genres =
                await context.Genres.OrderBy(x => x.Name).ToListAsync();

            var movieTheatersDTO =
                mapper.Map<List<MovieTheaterDTO>>(movieTheaters);
        ...
        Post([FromForm] MovieCreationDTO movieCreationDTO)
        {
        ...
        public async Task<ActionResult<MoviePutGetDTO>> PutGet(int id)
        {
            ...
            var genresSelectedIds =
                await context.Genres.Select(x => x.Id).ToListAsync();

            var nonSelectedGenres =
                context
                    .Genres
                    .Where(x => !genresSelectedIds.Contains(x.Id))
                    .ToList();

            var movieTheatersIds =
                movie.MovieTheaters.Select(x => x.Id).ToList();
            ...
            var nonSelectedGenresDTOs =
                mapper.Map<List<GenreDTO>>(nonSelectedGenres);
            var nonSelectedMovieTheatersDTO =
                mapper.Map<List<MovieTheaterDTO>>(nonSelectedMovieTheaters);
